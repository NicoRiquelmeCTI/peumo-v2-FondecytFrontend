<template>
    <div id="TabPalabrasClaves">
        <div class="wrapper">
            ...
            <b-modal id="modal-md-pclaves" size="md" title="PalabrasClaves" hide-footer>
                <div class="accordion" role="tablist" id="accordion2">
                    <b-card v-for="(item, index) in resPalabrasClaves2" v-bind:key="index" no-body class="mb-1">
                        ...
                        <b-collapse :id="`collapse-${ index }`" accordion="my-accordion" role="tabpanel">
                            <b-card-body>
                                <b-card-text v-html="item.paper_keywords"></b-card-text>
                            </b-card-body>
                        </b-collapse>
                    </b-card>
                </div>
...
export default {
    name: "TabPalabrasClaves",
    data() {
        return {
            resPalabrasClaves: null,
            resPalabrasClaves2: [{'title':'','paper_keywords':''}],
            patron: null,
            anio: 0,
            optionsTipoAnio: [
                { value: "null", label: "-" },
        ...
        async submitHandler() {
            ...
            try {
                ...
                let res = await axios.post(
                    `${process.env.VUE_APP_API_URL}/api/PClaves`,
                    formData
                );
                this.resPalabrasClaves = res.data.html_response;
                this.resPalabrasClaves2 = res.data.context2;
                this.makeToast(
